 ...
 * @generated
 */
public class HoldsAtItemSemanticEditPolicy extends GeneratorBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public HoldsAtItemSemanticEditPolicy() {
		super(GeneratorElementTypes.HoldsAt_2003);
	}
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (GeneratorVisualIDRegistry.getVisualID(incomingLink) == InitiallyHoldsAtsEditPart.VISUAL_ID) {
			...
			if (GeneratorVisualIDRegistry
					.getVisualID(incomingLink) == BehaviouralDescriptionHoldsAtsEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (GeneratorVisualIDRegistry.getVisualID(outgoingLink) == HoldsAtContextRelationEditPart.VISUAL_ID) {
