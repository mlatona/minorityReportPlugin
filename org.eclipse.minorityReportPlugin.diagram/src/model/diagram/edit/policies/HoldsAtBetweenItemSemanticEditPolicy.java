 ...
 * @generated
 */
public class HoldsAtBetweenItemSemanticEditPolicy extends GeneratorBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public HoldsAtBetweenItemSemanticEditPolicy() {
		super(GeneratorElementTypes.HoldsAtBetween_2011);
	}
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (GeneratorVisualIDRegistry.getVisualID(incomingLink) == InitiallyHoldsAtBetweensEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (GeneratorVisualIDRegistry
					.getVisualID(outgoingLink) == HoldsAtBetweenContextRelationEditPart.VISUAL_ID) {
