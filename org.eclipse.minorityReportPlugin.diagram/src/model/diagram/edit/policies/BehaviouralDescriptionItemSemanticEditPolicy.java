 ...
 * @generated
 */
public class BehaviouralDescriptionItemSemanticEditPolicy extends GeneratorBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public BehaviouralDescriptionItemSemanticEditPolicy() {
		super(GeneratorElementTypes.BehaviouralDescription_2010);
	}
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (GeneratorVisualIDRegistry
					.getVisualID(outgoingLink) == BehaviouralDescriptionHappensEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(outgoingLink.getSource().getElement(), null,
						outgoingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
			...
			if (GeneratorVisualIDRegistry
					.getVisualID(outgoingLink) == BehaviouralDescriptionHoldsAtsEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(outgoingLink.getSource().getElement(), null,
						outgoingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
	...
	protected Command getStartCreateRelationshipCommand(CreateRelationshipRequest req) {
		if (GeneratorElementTypes.BehaviouralDescriptionHoldsAtBetweens_4007 == req.getElementType()) {
			return getGEFWrapper(
					new BehaviouralDescriptionHoldsAtBetweensCreateCommand(req, req.getSource(), req.getTarget()));
		}
		if (GeneratorElementTypes.BehaviouralDescriptionHappens_4013 == req.getElementType()) {
			return getGEFWrapper(new BehaviouralDescriptionHappensCreateCommand(req, req.getSource(), req.getTarget()));
		}
		if (GeneratorElementTypes.ComplexEventBehaviouralDescriptions_4008 == req.getElementType()) {
			return null;
		}
		if (GeneratorElementTypes.BehaviouralDescriptionHoldsAts_4014 == req.getElementType()) {
			return getGEFWrapper(
