 ...
 * @generated
 */
public class HappensItemSemanticEditPolicy extends GeneratorBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public HappensItemSemanticEditPolicy() {
		super(GeneratorElementTypes.Happens_2009);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (GeneratorVisualIDRegistry
					.getVisualID(incomingLink) == BehaviouralDescriptionHappensEditPart.VISUAL_ID) {
			...
			if (GeneratorVisualIDRegistry.getVisualID(incomingLink) == InitiallyHappensEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (GeneratorVisualIDRegistry.getVisualID(outgoingLink) == HappensContextRelationEditPart.VISUAL_ID) {
